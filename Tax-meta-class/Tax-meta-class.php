...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
		...
		/**
		 * Show number field.
		 * 'step' set to 'any' to allow decimal (non-integer) numbers
		 *
		 * @param string $field
		 * @param string $meta
		 *
		...
		public function show_field_number( $field, $meta ) {
			$this->show_field_begin( $field, $meta );
			echo "<input type='text' pattern='[-+]?[0-9]*\\.?[0-9]+' class='at-number' name='{$field['id']}' id='{$field['id']}' value='{$meta}' style='{$field['style']}' size='30' />";
			$this->show_field_end( $field, $meta );
		}
